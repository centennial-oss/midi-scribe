.PHONY: build build-release-unsigned generate-appicons test clean generate-build-info lint lint-fix-safe
...
test:
	mkdir -p build
	xcodebuild test -scheme "$(XCODE_SCHEME)" -configuration Debug -derivedDataPath build/DerivedData -destination 'platform=macOS' CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO CODE_SIGNING_ALLOWED=NO
