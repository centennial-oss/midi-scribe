...
clean:
	@rm -rf build dist ; rm -rf ~/Library/Developer/Xcode/DerivedData/MIDIScribe-*
...
APPICON_DIR := MIDIScribe/Assets.xcassets/AppIcon.appiconset
...
APPICON_TRANSPARENT_IMAGESET := MIDIScribe/Assets.xcassets/AppIconTransparent.imageset/app-icon-transparent.png
...
# Writes MIDIScribe/Version.xcconfig so the app shows this version in About and Info.plist.
TAGVER ?=

generate-build-info:
	@printf '// Generated by Makefile - do not edit\nenum BuildInfoGenerated {\n    static let buildCommit = "%s"\n    static let buildDate = "%s"\n    static let buildConfiguration = "%s"\n    static let buildArch = "%s"\n}\n' "$(COMMIT)" "$(DATE)" "$(CONFIG)" "$(ARCH)" > MIDIScribe/BuildInfo.generated.swift

build:
	mkdir -p build
	@if [ -n "$(TAGVER)" ]; then \
		printf 'MARKETING_VERSION = %s\nINFOPLIST_KEY_NSHumanReadableCopyright = Copyright © 2026 Centennial OSS\n' "$(TAGVER)" > MIDIScribe/Version.xcconfig; \
	fi
	xcodebuild -scheme MIDIScribe -configuration Debug -derivedDataPath build/DerivedData build CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO CODE_SIGNING_ALLOWED=NO
...
build-release-unsigned:
	...
	xcodebuild -scheme MIDIScribe -configuration Release -derivedDataPath dist/DerivedData build CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO CODE_SIGNING_ALLOWED=NO
	cp -R "dist/DerivedData/Build/Products/Release/MIDI Scribe.app" dist/

# Run Swift tests.
test:
	mkdir -p build
	xcodebuild test -scheme MIDIScribe -configuration Debug -derivedDataPath build/DerivedData -destination 'platform=macOS' CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO CODE_SIGNING_ALLOWED=NO
